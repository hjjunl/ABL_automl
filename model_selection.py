...
if __name__ == '__main__':
    ...
    lgb = LGBMModel(X, y)
    lgb.train(RandomSearch=True, cpu_cnt=int(cpu_cnt), cv = int(cv), trials=int(trials))

    print(f'***************  CatBoost Best Parameter Search (Random Search) ******************')
    cat = CatBoostModel(X, y)
    cat.train(RandomSearch=True, cpu_cnt=int(cpu_cnt), cv = int(cv), trials=int(trials))

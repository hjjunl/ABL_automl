...
class CatBoostModel:
    ...
    def train(self, Optuna=None, RandomSearch=None, opt_trials=1, params=None, gpu=False, gpu_id=0, cpu_cnt=1, save_model=False, file_name='CatBoostClassifier', random_state=42, cv=5):
        """
        ...
        if not Optuna:
            if not RandomSearch:
                ...
                if not gpu:
                    cbc_model = CatBoostClassifier(n_estimators = 5000, thread_count = int(cpu_cnt), random_state = random_state)
                    search = RandomizedSearchCV(cbc_model, params_rs, n_iter=500, scoring='f1_macro', n_jobs=cpu_cnt, cv=cv, random_state=42)
